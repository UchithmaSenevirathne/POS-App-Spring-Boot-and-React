...
export default function Sidebar({ role }) {
  ...
  const linksToRender =
    role === "ADM" ? ADMIN_SIDEBAR_LINKS : USER_SIDEBAR_LINKS;

  const handleLogout = () => {
    // Print localStorage data
    console.log("LocalStorage data before clearing:", localStorage);
  ...
  return (
    <Link
      ...
      className={classNames(
        "flex items-center gap-2 font-normal px-6 py-2 rounded-md text-base mx-3 transition-all duration-500 no-underline hover:no-underline",
        isActive
          ? "bg-[#fff3dd] text-[orange]"
          : "text-[#505050] hover:bg-[#fff3dd] hover:text-[orange]"
      )}
    >
      <span className="text-xl">{item.icon}</span>
      {item.label}
