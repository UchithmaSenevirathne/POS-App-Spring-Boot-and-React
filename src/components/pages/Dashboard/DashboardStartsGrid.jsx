    ...
    <div className="grid w-full grid-cols-5 gap-4 pb-4">
        {categories.map((category, index) => (
          ...
          isSelected={selectedCategory === category.name}>
            ...
            <div className='pl-4'>
              <span className='font-medium text-black text-[14px]'>{category.name}</span>
            </div>
          </BoxWrapper>
        ))}
      </div>
...
function BoxWrapper({ children, onClick, isSelected  }) {
    return (
        <div
          className={`flex items-center flex-1 p-4 border rounded-md cursor-pointer ${
            isSelected ? 'bg-[#fff3dd] border-[orange]' : 'bg-white hover:bg-[#fff3dd] hover:border-[orange] transition-all duration-500'
          }`}
          onClick={onClick}
        >
          {children}
